...
function AuthPage() {
  ...
  const handleSubmit = async (e: React.SyntheticEvent) => {
    ...
    try {
      ...
      const message = typedErr.response?.data?.message || typedErr.message || "Something went wrong.";
      ...
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-gray-900 border-2 border-rose-500 mb-4">
            {isLoginMode ? <LogIn size={28} className="text-rose-500" /> : <UserPlus size={28} className="text-rose-500" />}
          </div>
          <h1 className="text-3xl font-bold text-white">{isLoginMode ? "Welcome Back" : "Create Account"}</h1>
          <p className="text-gray-400 mt-2">{isLoginMode ? "Sign in to your kanban board" : "Sign up to get started"}</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-gray-900 rounded-xl border border-gray-700 p-6 shadow-2xl">
          ...
          {!isLoginMode && (
            <div className="mb-4">
              <label className="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-2 block">Name</label>
          ...
          <div className="mb-4">
            <label className="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-2 block">Email</label>
            <div className="relative">
          ...
          <div className="mb-6">
            <label className="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-2 block">Password</label>
            <div className="relative">
